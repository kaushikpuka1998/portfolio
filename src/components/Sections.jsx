import { metrics, experience, projects, skills, profile } from '../data'
        ...
        <div className="split">
          <div className="feat reveal">
            <div>
              ...
              <p>I write about Java internals, backend engineering and system design — Redis internals, write-through vs write-behind caching, order-matching engines, and large-scale system breakdowns with branded diagrams.</p>
            </div>
            <a href={profile.linkedin} target="_blank" rel="noreferrer" className="btn btn-ghost" style={{ alignSelf: 'flex-start' }}>Follow on LinkedIn →</a>
          </div>
          <div className="feat reveal">
            <div>
        ...
        <div className="skill-grid">
          {skills.map((s, i) => (
            <div className="skill-col reveal" key={i}>
              <h4>{s.group}</h4>
              <div className="items">{s.items.map((it, j) => <span className="chip" key={j}>{it}</span>)}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section id="contact">
      <div className="wrap">
        ...
        <p className="reveal">Open to backend & full-stack roles, system-design conversations, and collaborations. The inbox is always open.</p>
        <div className="contact-row reveal">
          <a href={`mailto:${profile.email}`} className="btn btn-primary">✉ Email me</a>
          <a href={profile.linkedin} target="_blank" rel="noreferrer" className="btn btn-ghost">in LinkedIn</a>
